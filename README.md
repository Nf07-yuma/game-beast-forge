...
- **クラウド同期（任意）**: 設定タブで「同期コード」を発行すると、モンスター・タマゴのデータがクラウドに保存され、別の端末でそのコードを入力すれば引き継げる（Firebaseプロジェクトの設定が必要。詳しくは下記）
...
このリポジトリの [`firestore.rules`](./firestore.rules) の内容を、Firebase console の「Firestore Database」→「ルール」にそのまま貼り付けて公開してください。ログイン不要の匿名認証だけで使えるようにするため、「同期コード自体が実質的な鍵になる」というシンプルなモデルを採用しています（詳しくはファイル内のコメント参照）。カジュアルなゲームデータ向けの割り切りであり、機密性の高い用途には使わないでください。
